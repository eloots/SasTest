...
object SASInterfaceHelpers {
  ...
  def sendTestData(
    ...
    val inputDelimitedFileName = s"${SASConfig.tempFileBaseFolder}/${UUID.randomUUID()}"
  ...
  def retrieveSasTable(sasTestOutputDataset: String,
    ...
    val outputDelimitedFileName = s"${SASConfig.tempFileBaseFolder}/${UUID.randomUUID()}"
    val SASexportCode =
      s"""proc export data=${sasTestOutputDataset} replace
